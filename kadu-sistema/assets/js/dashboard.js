...
(async function(){
  ...
  function renderKPIs(products, movements){
    ...
    const valorPotencial = products.reduce((s,p)=>s+totalQtd(p)*p.preco,0);
    ...
    const vendidosMes = unidadesVendidasNoMes(movements);
    const topVenda = camisasMaisVendidas(movements, products, 1)[0];
  ...
  function renderRankingMaisVendidas(movements, products){
    const top = camisasMaisVendidas(movements, products, 5);
  ...
  renderKPIs(products, movements);
  renderCharts(products);
  renderRankingMaisVendidas(movements, products);
