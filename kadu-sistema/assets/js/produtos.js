...
const TABLE_COLUMNS = [
  ...
  { key:'custo', label:'Custo', defaultOn:true },
];
let colVisibility = {};
...
function renderTable(){
  ...
  const headCols = ['Foto','Time','Categoria', ...TABLE_COLUMNS.filter(c=>colVisibility[c.key]).map(c=>c.label), 'Tamanhos','Preço','Status',''];
  ...
  if(pageItems.length === 0){
    ...
    body.innerHTML = pageItems.map(p=>{
      ...
      const extraCols = TABLE_COLUMNS.filter(c=>colVisibility[c.key]).map(c=>{
        if(c.key==='custo') return `<td class="mono">${formatBRL(p.custo)}</td>`;
      ...
      return `
        <tr>
          ...
          <td class="mono">${formatBRL(p.preco)}</td>
...
function openAddModal(){
  ...
  document.getElementById('lucroPreview').textContent = 'R$ 0,00';
  renderUploadArea();
  renderSizeGrid();
...
function atualizarLucroPreview(){
  const custo = Number(document.getElementById('inCusto').value) || 0;
  const preco = Number(document.getElementById('inPreco').value) || 0;
  document.getElementById('lucroPreview').textContent = formatBRL(preco - custo);
}
document.getElementById('inCusto').addEventListener('input', atualizarLucroPreview);
document.getElementById('inPreco').addEventListener('input', atualizarLucroPreview);
...
async function saveProduct(){
  ['f-nome','f-team','f-custo','f-preco'].forEach(id=>document.getElementById(id).classList.remove('invalid'));
  ...
  if(custo === '' || Number(custo) < 0){ document.getElementById('f-custo').classList.add('invalid'); valido = false; }
  if(preco === '' || Number(preco) < 0){ document.getElementById('f-preco').classList.add('invalid'); valido = false; }
