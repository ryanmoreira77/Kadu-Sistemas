...
function renderFinStats(products, movements){
  const valorInvestido = products.reduce((s,p)=>s+totalQtd(p)*p.custo,0);
  const valorEstoque = products.reduce((s,p)=>s+totalQtd(p)*p.preco,0);
  const lucroEstimado = valorEstoque - valorInvestido;
  const receita = receitaTotal(movements);
...
function renderMaisCaros(products){
  ...
  document.getElementById('rankMaisCaros').innerHTML = top.map((p,i)=>`
    <div class="rank-item">
      ...
      <div class="rank-info">
        ...
        <div class="sub">${p.marca || ''} • ${totalQtd(p)} unidade(s)</div>
      </div>
...
function renderParados(products, movements){
  document.getElementById('paradosSub').textContent = `Sem vendas nos últimos ${PARADO_DIAS} dias`;
  const parados = produtosParados(products, movements).slice(0,5);
  document.getElementById('rankParados').innerHTML = parados.map((p,i)=>`
    <div class="rank-item">
      ...
      <div class="rank-info">
        <div class="name">${escapeHtml(p.team)}</div>
        <div class="sub">${totalQtd(p)} unidade(s) em estoque</div>
      </div>
...
(async function(){
  await renderShell('financeiro');
  const products = await getProducts();
  const movements = await getMovements();
  renderFinStats(products, movements);
  renderMaisCaros(products);
  renderParados(products, movements);
})();
