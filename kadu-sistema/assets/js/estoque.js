...
async function registrarMov(){
  ...
  if(!produtoId){
    showToast('Cadastre um produto antes de registrar movimentações.', 'error');
    return;
  }
  ...
  const resultado = await registrarMovimentacao({ produtoId, tamanho, tipo, quantidade: qtd, motivo, observacao });
  if(!resultado.ok){
...
async function recarregarMov(){
  movCache = await getMovements();
  renderMovTable();
}
...
function getFilteredMovs(){
  ...
  return movCache
    .map(m => ({ ...m, produto: produtosCache.find(p=>p.id===m.produtoId) }))
    .filter(m => m.produto)
    .filter(m=>{
