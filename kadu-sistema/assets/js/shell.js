/* =========================================================
   KADU CAMISAS DE TIME — Sidebar (6 seções) e Header compartilhados
   ========================================================= */
const NAV_ICONS = {
...
const NAV_ITEMS = [
  ...
  { key:'estoque',    label:'Estoque',     file:'estoque.html' },
  { key:'financeiro', label:'Financeiro',  file:'financeiro.html' },
  { key:'relatorios', label:'Relatórios',  file:'relatorios.html' },
  { key:'catalogo',   label:'Catálogo',    file:'catalogo.html' },
  { key:'config',     label:'Configurações', file:'configuracoes.html' },
];

const PAGE_META = {
  dashboard:  { title:'Dashboard',   sub:'Visão geral do seu estoque e das vendas' },
  produtos:   { title:'Produtos',    sub:'Cadastro e controle de camisas em estoque' },
  estoque:    { title:'Estoque',     sub:'Entradas, saídas, vendas e ajustes' },
  financeiro: { title:'Financeiro',  sub:'Valores investidos, receita e lucro' },
  relatorios: { title:'Relatórios',  sub:'Exporte seus dados em PDF, Excel ou CSV' },
...
async function renderShell(active){
  const session = await requireAuthCloud();
  if(!session) return; // já redirecionado para o login
  ...
  document.getElementById('sidebar-root').innerHTML = `
    <aside class="sidebar" id="sidebarEl">
      ...
      <div class="sidebar-footer">
        <button class="nav-item logout" id="logoutBtn">${NAV_ICONS.logout}Sair</button>
      </div>
    </aside>`;
  ...
  document.getElementById('header-root').innerHTML = `
    <header class="topheader">
      ...
      <div class="user-chip">
        <span class="user-avatar">${userInitials(email)}</span>
        <span>${email}</span>
      </div>
    </header>`;
