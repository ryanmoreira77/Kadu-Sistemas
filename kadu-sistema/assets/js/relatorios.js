/* =========================================================
   ...
   ========================================================= */
(async function(){ await renderShell('relatorios'); })();
...
async function getReportData(type){
  ...
  if(type === 'estoque'){
    ...
    const rows = getEstoqueLinhas(produtos).map(l=>[
  ...
  if(type === 'vendas'){
    ...
    const vendas = movements.filter(m=>m.tipo==='Venda').sort((a,b)=>new Date(b.data)-new Date(a.data));
...
async function baixarRelatorio(type, formato){
  const { headers, rows } = await getReportData(type);
