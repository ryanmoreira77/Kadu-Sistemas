...
async function gerarCatalogoPdf(){
  ...
  itens.forEach((p, i)=>{
    ...
    try{
      doc.addImage(p.foto, getImageFormat(p.foto), x + (cellW-imgSize)/2, y, imgSize, imgSize);
    }catch(e){
    ...
    doc.setFontSize(9);
    doc.setTextColor(120,120,120);
    const tamanhosComQtd = tamanhosResumo(p);
    doc.text(`Tamanhos: ${tamanhosComQtd}`, x + cellW/2, y + imgSize + 17.5, { align:'center', maxWidth: cellW-6 });
  });
