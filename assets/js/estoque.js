/* =========================================================
   KADU CAMISAS DE TIME — Estoque (movimentações)
   ========================================================= */
renderShell('estoque');
...
function popularSelectProdutos(){
  ...
  select.innerHTML = produtosCache
    ...
    .map(p=>`<option value="${p.id}">${escapeHtml(p.team)} — ${p.categoria} ${p.tamanho} (${p.sku})</option>`)
    .join('');
  atualizarEstoqueAtual();
}

function atualizarEstoqueAtual(){
  const id = Number(document.getElementById('movProduto').value);
  ...
  if(produto){
    hint.textContent = `Estoque atual: ${produto.qtd} unidade(s)`;
  }
...
function registrarMov(){
  ...
  const resultado = registrarMovimentacao({ produtoId, tipo, quantidade: qtd, motivo, observacao });
  if(!resultado.ok){
  ...
  showToast(`Movimentação registrada! Novo estoque: ${resultado.novaQtd} unidade(s).`, 'success');
  movPage = 1;
  renderMovTable();
}
...
function renderMovTable(){
  ...
  if(pageItems.length === 0){
    body.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-dim);padding:40px 10px;">Nenhuma movimentação encontrada.</td></tr>`;
  } else {
    body.innerHTML = pageItems.map(m=>`
      <tr>
        <td class="mono">${formatDataBR(m.data)}</td>
        <td>${escapeHtml(m.produto.team)} <span style="color:var(--text-dim);font-size:12px;">${m.produto.tamanho}</span></td>
...
/* ===== eventos ===== */
document.getElementById('movProduto').addEventListener('change', atualizarEstoqueAtual);
