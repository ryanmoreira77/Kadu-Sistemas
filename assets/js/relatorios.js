...
function getReportData(type){
  ...
  if(type === 'estoque'){
    ...
    const rows = produtos.map(p=>[
      p.team, p.categoria, p.tamanho, p.sku, p.qtd,
      p.qtd===0 ? 'Esgotado' : (p.qtd<=LOW_STOCK ? 'Estoque baixo' : 'Em estoque')
    ]);
    return { headers, rows };
  }

  if(type === 'produtos'){
    const headers = ['Nome','Time','Liga','Temporada','Marca','Modelo','Categoria','Tamanho','Código','Custo (R$)','Venda (R$)'];
    const rows = produtos.map(p=>[
      p.nome||'', p.team, p.liga||'', p.temporada||'', p.marca||'', p.modelo||'',
      p.categoria, p.tamanho, p.sku, p.custo.toFixed(2), p.preco.toFixed(2)
    ]);
    return { headers, rows };
  }

  if(type === 'lucro'){
    const headers = ['Time','Tamanho','Custo unit. (R$)','Venda unit. (R$)','Lucro unit. (R$)','Quantidade','Lucro potencial (R$)'];
    const rows = produtos.map(p=>{
      const lucroUnit = p.preco - p.custo;
      return [p.team, p.tamanho, p.custo.toFixed(2), p.preco.toFixed(2), lucroUnit.toFixed(2), p.qtd, (lucroUnit*p.qtd).toFixed(2)];
    });
    return { headers, rows };
  }
