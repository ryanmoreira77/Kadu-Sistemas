...
function renderKPIs(products){
  ...
  const estoqueBaixo = getEstoqueLinhas().filter(l=>l.qtd>0 && l.qtd<=LOW_STOCK).length;
...
function renderLowStockTable(){
  const linhas = getEstoqueLinhas().filter(l=>l.qtd<=LOW_STOCK).sort((a,b)=>a.qtd-b.qtd);
